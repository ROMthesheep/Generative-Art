  ...
  function draw() {
    ...
    for (var x = 0; x < rows; x++) {
      ...
      for (var y = 0; y < cols; y++) {
        ...
        flowfield[index] = v;
        colorine[index] = [map(noise(xoff),0,1,0,255),map(noise(yoff),0,1,0,255),map(noise(zoff),0,1,0,255)]
        v.setMag(3);
        xoff += inc;
        // fill(r);
    ...
    for (var i = 0; i < particulas.length; ++i) {
      ...
      particulas[i].update();
      particulas[i].show(colorine[index][0],colorine[index][1],colorine[index][2]);
      particulas[i].edges();
    }
